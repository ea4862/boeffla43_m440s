...
#ifdef SUPPORT_USB_TX_ACK_MULTIPACKETS
#define MAX_TX_URB	(512)
#define MAX_ACK_SIZE (64)

static int usb_tx_urb_with_skbs(struct usb_link_device *usb_ld,
		struct sk_buff_head *skb_list, struct if_usb_devdata *pipe_data);
#endif
		...
		struct sk_buff_head *skb_list, struct if_usb_devdata *pipe_data)
{
	...
	while(skb_list->qlen) 
	{
		...
		if (skb)
		{
			if (tx_len == 0 && (skb->len == 0 || skb->len > MAX_ACK_SIZE)) {
				tx_buffer = skb->data;
				tx_len = skb->len;
				break;
			} else if (skb->len == 0 || skb->len > MAX_ACK_SIZE || MAX_TX_URB < (tx_len + skb->len)) {
				skb_queue_head(skb_list, skb); // next time
				skb = NULL;
				break;
			}
			
			memcpy(tx_buffer+tx_len, skb->data, skb->len);
			tx_len += skb->len;
	...
	usb_fill_bulk_urb(urb, usbdev, pipe_data->tx_pipe, tx_buffer,
			tx_len, usb_tx_complete, (void *)skb);
...
#ifndef SUPPORT_USB_TX_ACK_MULTIPACKETS
...
#else
		if (ld->sk_raw_tx_q.qlen) {
			pipe_data = &usb_ld->devdata[IF_USB_RAW_EP];
			ret = usb_tx_urb_with_skbs(usb_ld, &ld->sk_raw_tx_q, pipe_data);
			if (ret < 0) {
				mif_err("usb_tx_urb_with_skbs "
						"for raw, ret(%d)\n",
						ret);
				return;
			}
...
#ifdef SUPPORT_USB_TX_ACK_MULTIPACKETS
	usb_ld->tx_buff = kzalloc(MAX_TX_URB, GFP_KERNEL);
	if (!usb_ld->tx_buff)
		goto err;
#endif
